...
export function calculateDistribution(input: DistributionInput): DistributionResult {
  ...
  for (const emp of included) {
    ...
    const total_tax = contractor_tax_pkr + remittance_tax_pkr;
    const net_pkr = gross_pkr - total_tax;

    total_threshold_savings += emp.salary_usd * emp.threshold;
  ...
  const company_total_before_tax = company_gross_from_usd + total_threshold_savings;
